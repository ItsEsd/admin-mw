...
var rsign =
  "AKfycbyrwGmu2xLppdpLnGownMSbh1gsIiJxizzpUSnJs6TerlByKqt4i24NSC8t7nRf_i3Ejw/exec";
...
$('form[id="mwall_inmedia#1"]').submit(function () {
  var TOD, k;
  document.getElementById("inmedia1_json").value = encodeURIComponent(
    JSON.stringify($('form[id="mwall_inmedia#1"]').serializeObject()),
  );
  ...
  $("#mwall_inmedia1").show();
  document.getElementById("mwall_inmedia1").innerHTML +=
    '<p style="color:black;">' +
    TOD.conTitle1 +
...
$('form[id="mwall_inmedia#2"]').submit(function () {
  var TOD, k;
  document.getElementById("inmedia2_json").value = encodeURIComponent(
    JSON.stringify($('form[id="mwall_inmedia#2"]').serializeObject()),
  );
  ...
  $("#mwall_inmedia2").show();
  document.getElementById("mwall_inmedia2").innerHTML +=
    '<p style="color:black;">' +
    TOD.conTitle1 +
    '<a target="_blank" class="readmore" href="' +
    TOD.conLink1 +
...
document
  .getElementById("postn2")
  .addEventListener("click", update_value_inmtwo);
...
$(document).ready(function () {
  ...
  $(add_button).on("click", function (e) {
    ...
    if (x < max_vids) {
      x++;
      $(wrapper).append(
        `<div class="slide"><hr style="border-color:red;border-width:1px;width:40%;"> <div> <input autocomplete="off" name="inmediamwallVid" class="form-control" id="youvlink" placeholder="Youtube Video ID" class="secvid" oninput="enablePreview()" required/></div><a href="#" class="remove_field">Remove</a><br></div>`,
      );
    }
    if (x == max_vids) {
      $("#addv").hide();
...
$(function () {
  $('form[id="mwallinmediavid"]').submit(function () {
    document.getElementById("frameVid_json").value = JSON.stringify(
      $('form[id="mwallinmediavid"]').serializeObject(),
    );
...
function previewVid() {
  ...
  if (error1 == 1 && error2 == 1) {
    document.getElementById("preview_youv").innerHTML =
      "<br><p class='addmorecon'>Update or Add One More Video</p><br>";
  } else {
    for (j; j < maxTOD; j++) {
      var link = VID.inmediamwallVid[j];
      var vidId = link;
      document.getElementById("preview_youv").innerHTML +=
        '<div class="iteam"><div class="embed-responsive embed-responsive-16by9"><iframe width="100%"class="embed-responsive-item" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; autoplay ;picture-in-picture" allowfullscreen src="//www.youtube.com/embed/' +
        vidId +
...
$(function () {
  $('form[id="quote_edit"]').submit(function () {
    document.getElementById("quotes_json").value = encodeURIComponent(
      JSON.stringify($('form[id="quote_edit"]').serializeObject()),
    );
    document.getElementById("quotes_json_unesc").value = JSON.stringify(
      $('form[id="quote_edit"]').serializeObject(),
    );
...
function previewQuote() {
  ...
  for (j; j < maxQ; j++) {
    document.getElementById("preview_quote").innerHTML +=
      '<div style="border:1px solid #0c29cd;margin-top:10px;border-radius:6px;max-width:400px;"><p style="color:#0c29cd;">' +
      Quote.dquoteWall[j] +
      "</p></div>";
  }
}
...
$(function () {
  $('form[id="exinfo_edit"]').submit(function () {
    document.getElementById("exinfo_json").value = encodeURIComponent(
      JSON.stringify($('form[id="exinfo_edit"]').serializeObject()),
    );
    document.getElementById("exinfo_json_unesc").value = JSON.stringify(
      $('form[id="exinfo_edit"]').serializeObject(),
    );
...
function previewExinfo() {
  ...
  document.getElementById("preview_exinfo").innerHTML +=
    '<div class="eninpresec"><p>' + Exin.exminfo + "</p></div>";
}

document
  .getElementById("postexin")
...
document.addEventListener("DOMContentLoaded", function () {
  const formlogn = document.getElementById("tmadmnfrm");
  formlogn.addEventListener("submit", function (event) {
